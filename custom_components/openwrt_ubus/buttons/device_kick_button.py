...
async def async_setup_entry(
    ...
    button_entities: dict[str, DeviceKickButton] = {}  # Store references to button entities for availability updates
    ...
    def _async_add_kick_buttons():
        ...
        new_buttons: list[DeviceKickButton] = []
        ...
        for mac, device_info in device_statistics.items():
            ap_device = device_info.ap_device
            if not ap_device:
                continue
            ...
            if button_id not in created_buttons:
                ...
                kick_button = DeviceKickButton(
                    ...
                    device_mac=mac,
                    device_name=device_info.hostname or f"Device {mac}",
                    unique_id=button_id
                )
                ...
                _LOGGER.debug("Created kick button for device %s (%s) on AP %s",
                              device_info.hostname or mac, mac, ap_device)
...
class DeviceKickButton(CoordinatorEntity[SharedDataUpdateCoordinator], ButtonEntity):
    ...
    def available(self) -> bool:
        ...
        if not super().available:
            ...
            if not self.coordinator.data.get("hostapd_available", False):
                _LOGGER.debug("Button %s: Hostapd not available", self._attr_unique_id)
                current_state = False
            else:
                # Check if device is still connected
                device_info = self.coordinator.data.get("device_statistics", {}).get(self._device_mac, {})
    ...
    async def async_press(self) -> None:
        ...
        try:
            ...
            raise HomeAssistantError(f"Failed to kick device {self._device_name}") from exc
