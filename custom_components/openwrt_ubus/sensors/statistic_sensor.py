...
from .. import SharedUbusDataManager
from ..const import (
    DOMAIN,
    CONF_STATION_SENSOR_INTERVAL,
    DEFAULT_STATISTIC_SENSOR_INTERVAL,
)
from ..shared_data_manager import SharedDataUpdateCoordinator, SharedData
...
def _get_simple_value(device_data: dict, keys: list[str]) -> Any:
...
def _calculate_speed(device_data: dict, keys: list[tuple], sensor_instance=None) -> float | None:
...
async def async_setup_entry(
    ...
    interval = entry.options.get(
        CONF_STATION_SENSOR_INTERVAL,
        entry.data.get(CONF_STATION_SENSOR_INTERVAL, DEFAULT_STATISTIC_SENSOR_INTERVAL)
    )
    ...
    async def _handle_coordinator_update_async():
        ...
        # Handle new devices
        if new_devices := current_devices - coordinator.known_devices:
...
class DeviceStatisticsSensor(CoordinatorEntity[SharedDataUpdateCoordinator], SensorEntity):
    ...
    def _get_device_name(self) -> str:
        ...
        if device_data:
            ...
            if hostname and hostname != self.mac_address and hostname != self.mac_address.upper() and hostname != "*":
                return f"{connected_router} {hostname}"
